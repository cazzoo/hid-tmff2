...
if lsmod | grep -q "^hid_tmff_new " || grep -q "^hid_tmff_new " /proc/modules; then
    echo "  - Removing hid_tmff_new..."
    modprobe -r hid_tmff_new 2>/dev/null || {
...
if [ -f "$LOCAL_KO" ]; then
    ...
    # Try to insert module, but handle "already loaded" gracefully
    if insmod "$LOCAL_KO" "$@" 2>/dev/null; then
        echo -e "${GREEN}  ✓ hid_tmff_new loaded from local build${NC}"
    else
        # Check if failure was because module already exists
        if lsmod | grep -q "^hid_tmff_new " || grep -q "^hid_tmff_new " /proc/modules; then
            echo -e "${YELLOW}  ⚠ Module already loaded (possibly auto-loaded by udev)${NC}"
            echo "  - Attempting to reload..."
            modprobe -r hid_tmff_new 2>/dev/null || rmmod -f hid_tmff_new 2>/dev/null || true
            sleep 0.5
            if insmod "$LOCAL_KO" "$@"; then
                echo -e "${GREEN}  ✓ hid_tmff_new reloaded from local build${NC}"
            else
                echo -e "${RED}  ✗ Failed to reload hid_tmff_new${NC}"
                exit 1
            fi
        else
