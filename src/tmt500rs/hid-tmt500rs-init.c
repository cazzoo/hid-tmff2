...
int t500rs_init_wheel(struct t500rs_device_entry *t500rs, int open_mode)
{
    ...
    msleep(200);  // Increased delay for cleanup
    ...
    if (error) {
        dev_err(&t500rs->data->hdev->dev, "Failed to initialize USB: %d\n", error);
        goto err_usb;
    }

    /* Wait for USB to stabilize */
    msleep(1000);  // Increased delay for USB stability
    ...
    if (error) {
        dev_err(&t500rs->data->hdev->dev, "Failed to detect mode: %d\n", error);
        goto err_mode;
    }
    ...
    if (current_mode != TMT500RS_MODE_WHEEL) {
        ...
        if (error) {
            dev_err(&t500rs->data->hdev->dev, "Failed to switch to wheel mode: %d\n", error);
            goto err_mode;
        }
        ...
        if (error) {
            dev_err(&t500rs->data->hdev->dev, "Failed to verify wheel mode: %d\n", error);
            goto err_mode;
        }
    }
    ...
    if (error) {
        dev_err(&t500rs->data->hdev->dev, "Failed to initialize device state: %d\n", error);
        goto err_init;
    }
    ...
    if (error) {
        dev_err(&t500rs->data->hdev->dev, "Failed to initialize force feedback: %d\n", error);
        goto err_ff;
    }
...
err_init:
    ...
    t500rs_send_command(t500rs, 0x0f, 0x01, 0x01);
err_mode:
