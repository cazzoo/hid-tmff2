...
static int t500rs_upload_constant(struct t500rs_device_entry *t500rs,
  ...
  t500rs_fill_envelope_u02(buf, &effect->u.constant.envelope, env_sub_second);
  ret = t500rs_send_usb(t500rs, buf, 9);
  if (ret) {
  ...
  /* Report 0x01 - Main effect upload (second; references second envelope subtype) */
  {
    ...
    m->b9 = param_sub;       /* Parameter subtype reference (per-effect) */
    m->b10 = 0x00;
    m->b11 = env_sub_second; /* Envelope subtype reference (second) */
    m->b12 = 0x00;
    m->b13 = 0x00;
    m->b14 = 0x00;
  }
