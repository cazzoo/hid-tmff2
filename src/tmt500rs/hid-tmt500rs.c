...
struct t500rs_device_entry {
	...
	/*
	 * Gain/autocenter control worker. The parent core invokes
	 * set_gain/set_autocenter directly from input event context (which
	 * can run with IRQs disabled) and from sysfs; those contexts cannot
	 * perform the blocking HID output this wheel requires. Store the
	 * latest request and apply it from process context, with a dedicated
	 * DMA-safe buffer so it never races the core FFB worker's
	 * send_buffer.
	 */
	...
	u16 ctrl_gain;
	u16 ctrl_autocenter;
	bool ctrl_gain_pending;
	bool ctrl_autocenter_pending;
...
static int t500rs_set_gain(void *data, u16 gain)
{
	...
	/* Defer to the control worker: may be called from atomic context */
	spin_lock_irqsave(&t500rs->ctrl_lock, flags);
	t500rs->ctrl_gain = gain;
	t500rs->ctrl_gain_pending = true;
	spin_unlock_irqrestore(&t500rs->ctrl_lock, flags);

	schedule_work(&t500rs->ctrl_work);
	return 0;
}

static int t500rs_send_gain_now(struct t500rs_device_entry *t500rs, u16 gain)
{
	u8 *buf;
	u8 device_gain_byte;
	int ret;

	if (!t500rs->ctrl_buffer) {
		hid_err(t500rs->hdev, "t500rs_send_gain_now: NULL ctrl buffer\n");
		return -ENOMEM;
	}

	buf = t500rs->ctrl_buffer;
...
static int t500rs_set_autocenter(void *data, u16 autocenter)
{
	...
	/* Defer to the control worker: may be called from atomic context */
	spin_lock_irqsave(&t500rs->ctrl_lock, flags);
	t500rs->ctrl_autocenter = autocenter;
	t500rs->ctrl_autocenter_pending = true;
	spin_unlock_irqrestore(&t500rs->ctrl_lock, flags);
...
static int t500rs_send_autocenter_now(struct t500rs_device_entry *t500rs,
				      u16 autocenter)
{
	u8 *buf;
	int ret;
...
/* Apply pending gain/autocenter requests in process context */
static void t500rs_ctrl_work(struct work_struct *w)
{
	struct t500rs_device_entry *t500rs =
		container_of(w, struct t500rs_device_entry, ctrl_work);
	unsigned long flags;
	u16 gain, autocenter;
	bool do_gain, do_autocenter;

	spin_lock_irqsave(&t500rs->ctrl_lock, flags);
	do_gain = t500rs->ctrl_gain_pending;
	gain = t500rs->ctrl_gain;
	t500rs->ctrl_gain_pending = false;
	do_autocenter = t500rs->ctrl_autocenter_pending;
	autocenter = t500rs->ctrl_autocenter;
	t500rs->ctrl_autocenter_pending = false;
	spin_unlock_irqrestore(&t500rs->ctrl_lock, flags);

	if (do_gain)
		t500rs_send_gain_now(t500rs, gain);

	if (do_autocenter)
		t500rs_send_autocenter_now(t500rs, autocenter);
}
...
static int t500rs_wheel_init(struct tmff2_device_entry *tmff2, int open_mode)
{
	...
	t500rs->ctrl_buffer = kzalloc(t500rs->buffer_length, GFP_KERNEL);
	if (!t500rs->ctrl_buffer) {
		hid_err(tmff2->hdev,
			"Failed to allocate ctrl buffer (%zu bytes)\n",
			t500rs->buffer_length);
		ret = -ENOMEM;
		goto err_ctrl_alloc;
	}
	...
	spin_lock_init(&t500rs->ctrl_lock);
	INIT_WORK(&t500rs->ctrl_work, t500rs_ctrl_work);
