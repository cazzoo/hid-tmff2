...
/* HID timeout - use a reasonable timeout for HID reports */
...
struct t500rs_pkt_r01_main {
	u8 id;             /* b0: 0x01 */
	u8 effect_id;      /* b1: hardware effect slot ID */
...
static void t500rs_build_r01_main(struct t500rs_pkt_r01_main *p,
  ...
  memset(p, 0, sizeof(*p));
  p->id = 0x01;
  p->effect_id = effect_id;       /* Hardware effect slot ID */
 ...
 *
 * CRITICAL WORKAROUND: Per Windows captures, BOTH X-axis and Y-axis packets have
...
static int t500rs_upload_constant(struct t500rs_device_entry *t500rs,
   ...
   */
  {
    ...
    t500rs_build_r01_main(m, 0, 0x00, duration_ms, delay_ms, param_sub, env_sub);
  }
...
static int t500rs_upload_periodic(struct t500rs_device_entry *t500rs,
  ...
  buf[0] = 0x42;
  buf[1] = 0x05;
  t500rs_send_hid(t500rs, buf, 2); /* Ignore errors - these are optional sync */

  buf[0] = 0x42;
  buf[1] = 0x04;
  t500rs_send_hid(t500rs, buf, 2);
  ...
  ret = t500rs_send_hid(t500rs, buf, sizeof(struct t500rs_pkt_r04_periodic_ramp));
  if (ret) {
...
static int t500rs_upload_ramp(struct t500rs_device_entry *t500rs,
  ...
  {
    struct t500rs_pkt_r01_main *m = (struct t500rs_pkt_r01_main *)buf;
    t500rs_build_r01_main(m, 0, 0x24, duration_ms, delay_ms, param_sub, env_sub);
  }
  ret = t500rs_send_hid(t500rs, buf, sizeof(struct t500rs_pkt_r01_main));
  if (ret) {
    hid_err(t500rs->hdev, "Failed to send Report 0x04 (ramp): %d\n", ret);
...
static int t500rs_wheel_init(struct tmff2_device_entry *tmff2, int open_mode) {
  ...
  hid_info(t500rs->hdev,
           "T500RS initialized successfully (HID mode)\n");
