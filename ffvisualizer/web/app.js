...
function connect() {
  ws = new WebSocket((location.protocol === "https:" ? "wss://" : "ws://")
                     + location.host + "/ws");
  ws.onopen = () => { wsState.className = "dot on"; };
  ws.onclose = () => { wsState.className = "dot off"; setTimeout(connect, 1000); };
  ws.onerror = () => { ws.close(); };
  ws.onmessage = (ev) => {
...
function setStrength(v) {
  ...
  $("strength-fill").style.height = Math.min(100, pct) + "%";
  $("strength-val").textContent = pct + "%";
}
function setDirection(deg, torque) {
  // Show torque sign as a left/right bar (more meaningful than the raw angle).
  const t = Math.max(-1, Math.min(1, torque));
  const fill = $("dir-fill");
  if (t >= 0) {
    fill.style.left = "50%"; fill.style.width = (t * 50) + "%";
  } else {
    fill.style.left = (50 + t * 50) + "%"; fill.style.width = (-t * 50) + "%";
  }
  $("dir-val").textContent = Math.round(deg) + "°";
}
...
async function loadGains() {
  ...
  try {
    ...
    gains = j.gains || {};
    $("gains-note").textContent = j.observe
      ? "(observe mode)" : "";
...
function renderGains(gains) {
  ...
  if (!keys.length) {
    host.innerHTML = '<span class="muted">No driver gain sysfs attributes found '
      + '(driver-agnostic: appears only for drivers exposing spring_level / '
      + 'damper_level / friction_level / gain).</span>';
    gainsBuilt = true;
    return;
  }
  host.innerHTML = "";
  for (const key of keys) {
    ...
    row.className = "gain-row" + (g.writable ? "" : " disabled");
    row.innerHTML =
      `<label>${g.name}</label>
    ...
    input.addEventListener("input", () => {
      gv.textContent = input.value + g.unit;
    });
