...
| 0x02 | Envelope | 9 bytes | Attack/fade parameters |
...
**⚠️ CRITICAL FINDING (2025-11-28):** Windows sends **zero coefficients, deadband, and center** in ALL 0x05 packets!
...
### 1. Packet Code Discrepancy - CONFIRMED
**CRITICAL:** The Windows captures consistently show code **0x2a** in 0x04 periodic packets and 0x05 conditional packets, but the current driver uses **0x0e**.

**Findings from Windows USB Captures:**
- **0x04 Periodic packets:** ALL use code 0x2a (not 0x0e)
  - Example: `04 2a 09 00 7f 64 00 00` (sine wave)
  - Current driver incorrectly uses 0x0e
- **0x05 Conditional packets:** Use codes 0x2a and 0x38 (two packets per effect)
  - First packet: `05 2a ...` (X-axis parameters)
  - Second packet: `05 38 ...` (Y-axis parameters)
- **0x01 Main upload packet bytes 9-10:** Indicate effect type
  - 0x000e = Constant force effect → expect 0x03 packet
  - 0x002a = Periodic/Conditional effect → expect 0x04 or 0x05 packets
- **0x01 Main upload packet bytes 11-12:** Indicate which packets follow
  - 0x001c = Envelope packet (0x02) follows
  - 0x0038 = Two conditional packets (0x05 with 0x2a and 0x05 with 0x38) follow

**Action Required:**
1. Change `struct t500rs_r04_periodic` code from 0x0e to **0x2a**
2. Implement `struct t500rs_r05_conditional` with two packets (codes 0x2a and 0x38)
3. Update 0x01 packet bytes 9-10 to use 0x002a for periodic/conditional effects
4. Update 0x01 packet bytes 11-12 to use 0x0038 for conditional effects

### 2. Period Encoding
**IMPORTANT:** Keep period in **milliseconds**, do NOT convert to Hz×100!
- Memory note states: "For T500RS periodic effects, keep period in milliseconds (no Hz×100 conversion)"
- Windows captures confirm: period values match milliseconds directly
- **Action Required:** Ensure driver does not apply frequency conversion.

### 3. Effect ID and Slot Management
**IMPORTANT:** The T500RS hardware supports up to 16 simultaneous effects with internal mixing.
- **Single Effect:** Windows driver uses effect_id = 0x0000 for simple single-effect scenarios
- **Multi-Effect:** Windows driver assigns unique effect IDs for concurrent effects
  - Observed IDs: 0x0000, 0x0021, 0x0022, 0x0040, 0x0041
  - Pattern is NOT sequential (0, 1, 2, 3...) but uses specific values
  - Each concurrent effect must have a unique ID
- **Hardware Behavior:**
  - Device maintains up to 16 effect slots internally
  - Device automatically mixes/sums forces from all active effects
  - Each effect is controlled independently via its effect ID
...
## Recommended Driver Implementation Sequence
...
**Analyzed Captures:**
- `captures/test_cases_minimal/windows_20251121_173433/` - 2 basic constant tests
- `captures/test_cases/windows_20251125_002122/` - 30+ comprehensive tests
- `captures/test_cases/windows_20251121_174637/` - Additional constant tests
- `captures/test_cases_multi_effect/windows_20251121_173511/` - Sequential effects
- `captures/test_cases_multi_effect/windows_20251122_005954/` - Overlapping effects
- `captures/test_cases_multi_effect/windows_20251125_093022/` - Multi-effect tests
- `captures/test_cases_multi_effect/windows_20251125_094350/` - Multi-effect tests
- `captures/test_cases_multi_effect_timing/windows_20251121_175858/` - Rapid sequential
- `captures/test_cases_multi_effect_timing/windows_20251125_093237/` - Timing tests

**Total Tests Analyzed:** 50+ individual effect tests across all categories
...
**Primary Captures Analyzed:**
- `captures/test_cases/windows_20251125_002122/` - 30+ comprehensive effect tests (primary reference)
- `captures/test_cases_multi_effect/windows_20251125_093022/` - Multi-effect concurrent tests
- `captures/test_cases_multi_effect/windows_20251125_094350/` - Additional multi-effect tests
- `captures/test_cases_parameter_scaling/windows_20251125_160952/` - Conditional parameter tests
- `captures/test_cases_effect_id_range/windows_20251125_160708/` - Effect ID pattern tests
- `captures/test_cases_square_wave/windows_20251125_161251/` - Square wave support test (negative result)
- `captures/test_cases_gain_interaction/windows_20251125_163520/` - Gain behavior tests
- `captures/test_cases_ramp_encoding/windows_20251125_161135/` - Ramp encoding tests

**Supporting Captures:**
- `captures/test_cases_minimal/windows_20251121_173433/` - Basic constant force tests
- `captures/test_cases_multi_effect_timing/windows_20251125_093237/` - Timing-focused tests

**Total Tests Analyzed:** 70+ individual effect tests across all categories

---

## Implementation Checklist for Driver

### Critical Changes Required

1. **✅ Update 0x01 Packet Structure:**
   - Change bytes 9-10 field name from `effect_code` to `packet_code_1`
   - Change bytes 11-12 field name from `envelope_flag` to `packet_code_2`
   - These are variable codes, not fixed values!

2. **✅ Update 0x04 Packet Structure:**
   - Change `code` field to be variable (from 0x01 bytes 9-10)
   - Remove Hz×100 conversion - period is in milliseconds
   - Current driver incorrectly uses 0x0e, should use value from 0x01 packet

3. **✅ Implement 0x05 Conditional Packet:**
   - Create new `struct t500rs_r05_conditional` with 11-byte structure
   - Send TWO packets per conditional effect
   - First packet uses code from 0x01 bytes 9-10
   - Second packet uses code from 0x01 bytes 11-12

4. **✅ Implement Effect ID Management:**
   - Track active effect slots (0-15 available)
   - Assign unique IDs to concurrent effects
   - Can use simple sequential assignment (0x00, 0x01, 0x02, etc.)
   - Windows uses non-sequential but any ID in range should work

5. **✅ Remove Square Wave Support:**
   - Square wave is NOT supported by T500RS hardware
   - Return error or map to triangle wave

6. **✅ Verify Scaling Formulas:**
   - Constant: `device_level = (sdl_level * 255 / 65535) - 127`
   - Periodic magnitude: `device_mag = sdl_mag * 127 / 32767`
   - Periodic phase: `device_phase = (sdl_phase * 256 / 36000) & 0xff`
   - Envelope: `device_level = sdl_level * 255 / 32767`
...
1. **✅ Protocol Analysis Complete** - All major packet structures documented
2. **Update Driver Implementation** - Apply critical changes listed above
3. **Test Conditional Effects** - Verify spring/damper with varied parameters on hardware
4. **Test Ramp Effects** - Determine correct encoding through hardware testing
5. **Validate Multi-Effect** - Test concurrent effect playback
6. **Performance Testing** - Verify timing and responsiveness on real hardware
